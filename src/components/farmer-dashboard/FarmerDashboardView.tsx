...
export function FarmerDashboardView() {
  ...
  const [selectedQueryForDetails, setSelectedQueryForDetails] =
    useState<ForwardedQuery | null>(null);
        ...
        <div className="flex items-center gap-4">
          <div>
            <h1 className="text-xl font-bold tracking-tight text-foreground">
              {greeting()},{" "}
              <span className="text-primary">{user?.name || "Farmer"}</span>
            </h1>
          </div>
        </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                size="icon"
                className="relative rounded-full border-primary/20 bg-background/50 backdrop-blur-sm"
              >
            ...
            <DropdownMenuContent
              align="end"
              className="w-80 p-0 border-primary/10"
            >
        ...
        <div className="lg:col-span-1 flex flex-col gap-8">
          ...
          <Card className="border-primary/10 bg-primary/5 shadow-xl shadow-primary/5">
            <CardHeader>
              ...
              <CardDescription>
                Forward your AI chat to a human agricultural officer for
                personalized advice.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <CheckCircle2 className="h-3 w-3 text-primary" /> Expert
                  verification
                </div>
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <CheckCircle2 className="h-3 w-3 text-primary" /> Localized
                  knowledge
                </div>
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <CheckCircle2 className="h-3 w-3 text-primary" /> Timely
                  responses
                </div>
              </div>
            </CardContent>
            ...
            {queriesLoading ? (
              Array(2)
                ...
                .map((_, i) => (
                  <Card
                    key={i}
                    className="border-primary/5 shadow-sm animate-pulse h-32"
                  />
                ))
              ...
              <Card className="border-dashed border-2 border-primary/10 bg-muted/30 p-12 flex flex-col items-center justify-center text-center gap-4">
                ...
                <p className="text-muted-foreground max-w-xs text-sm">
                  No recent queries. Your forwarded questions will appear here.
                </p>
              </Card>
            ) : (
              recentQueries.map((query) => (
                <Card
                  key={query._id}
                  className="border-primary/5 hover:border-primary/20 transition-all hover:shadow-md group overflow-hidden"
                >
